...
def convert_rows(observations):
    for _, obs_time, team, level, health, links in observations:
        yield (datetime.datetime.utcfromtimestamp(obs_time), team, level, health, links)
...
def calculate_per_portal_stats(portal_id, observations):
    ...
    obs_time, team, level, health, links = zip(*convert_rows(observations))
    df = pd.DataFrame({
        "obs_time": pd.Series(obs_time),
        "team": pd.Series(team, dtype='uint8'),
        "level": pd.Series(level, dtype='uint8'),
        "health": pd.Series(health, dtype='uint8'),
        "links": pd.Series(links, dtype='uint8'),
        })
    df['flips'] = df['team'].diff().astype('bool')
    ...
    flipped_obs = df['flips'][df['flips'] == True]
    if flipped_obs.size == 1:
        # Never captured
        flip_count = 0
        ...
        flipped_obs = flipped_obs[1:]
        first_flip = flipped_obs.index[0]
        flip_count = flipped_obs.size
        average_td = overall_td / flip_count
        current_held_td = first_obs['obs_time'] - df.iloc[first_flip]['obs_time']
...
SELECT
   ...
   team, portal_level, portal_health, COALESCE(links, 0)
FROM (
   SELECT
      portal AS portal_id
...
""")
    logging.debug('Dispatched query.')
    ...
    per_portal_stats = []
    for portal_id, observations in itertools.groupby(cur, lambda x: x[0]):
